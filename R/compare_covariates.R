...
#' my_crs <- sf::st_crs("+proj=aea +lat_1=38 +lat_2=30 +lat_0=34 +lon_0=-73 +x_0=0
...
compare_covariates <- function(
  ...
  if (is.null(extrapolation.object)) {
    ...
    if (!all(required.args %in% names(extrapolation.object))) {
      missing.args <- required.args[which(
        !required.args %in% names(extrapolation.object)
      )]
      stop(paste0(
        "Missing input arguments: ",
        paste0(missing.args, collapse = ", ")
      ))
    }
  }
  ...
  if (is.null(n.covariates)) {
    combs <- purrr::map2(
      .x = rep(list(covariate.names), length(covariate.names)),
    ...
    combs <- purrr::map2(
      .x = rep(list(covariate.names), length(n.covariates)),
  ...
  exsum <- purrr::map(
    .x = extrap,
    .f = ~ summarise_extrapolation(
      extrapolation.object = .x,
      extrapolation = FALSE,
      mic = FALSE
    )
  ) %>%
  ...
  for (k in 1:length(exsum)) {
    ...
    if (!"univariate" %in% temp.names) {
      temp <- append(temp, list(univariate.n = 0))
    }
    if (!"combinatorial" %in% temp.names) {
      temp <- append(temp, list(combinatorial.n = 0))
    }
  ...
  if (extrapolation.type == "both") {
    ...
    min.univariate <- vars.univariate[which(
      vars.univariate == min(vars.univariate)
    )]
    max.univariate <- vars.univariate[which(
      vars.univariate == max(vars.univariate)
    )]

    varmin.univariate <- combs[which(
      vars.univariate == min(vars.univariate)
    )] %>%
      collapse.vars(.)

    varmax.univariate <- combs[which(
      vars.univariate == max(vars.univariate)
    )] %>%
      collapse.vars(.)
    ...
    min.combinatorial <- vars.combinatorial[which(
      vars.combinatorial == min(vars.combinatorial)
    )]
    max.combinatorial <- vars.combinatorial[which(
      vars.combinatorial == max(vars.combinatorial)
    )]

    varmin.combinatorial <- combs[which(
      vars.combinatorial == min(vars.combinatorial)
    )] %>%
      collapse.vars(.)

    varmax.combinatorial <- combs[which(
      vars.combinatorial == max(vars.combinatorial)
    )] %>%
      collapse.vars(.)
  ...
  if (extrapolation.type == "both") {
    if (!length(varmin.univariate) == length(varmax.univariate)) {
      if (length(varmin.univariate) < length(varmax.univariate)) {
        varmin.univariate <- c(
          ...
          rep(
            "-",
            times = length(varmax.univariate) - length(varmin.univariate)
          )
        )
        min.univariate <- c(
          min.univariate,
          rep("-", times = length(max.univariate) - length(min.univariate))
        )
      } else {
        varmax.univariate <- c(
          varmax.univariate,
          rep(
            "-",
            times = length(varmin.univariate) - length(varmax.univariate)
          )
        )
        max.univariate <- c(
          max.univariate,
          rep("-", times = length(min.univariate) - length(max.univariate))
        )
      }
    }
    ...
    if (!length(varmin.combinatorial) == length(varmax.combinatorial)) {
      if (length(varmin.combinatorial) < length(varmax.combinatorial)) {
        varmin.combinatorial <- c(
          varmin.combinatorial,
          rep(
            "-",
            times = length(varmax.combinatorial) - length(varmin.combinatorial)
          )
        )
        min.combinatorial <- c(
          min.combinatorial,
          rep(
            "-",
            times = length(max.combinatorial) - length(min.combinatorial)
          )
        )
      } else {
        varmax.combinatorial <- c(
          varmax.combinatorial,
          rep(
            "-",
            times = length(varmin.combinatorial) - length(varmax.combinatorial)
          )
        )
        max.combinatorial <- c(
          max.combinatorial,
          rep(
            "-",
            times = length(min.combinatorial) - length(max.combinatorial)
          )
        )
      }
    }
    ...
    if (!length(varmin) == length(varmax)) {
      if (length(varmin) < length(varmax)) {
        ...
        min.both <- c(
          min.both,
          rep("-", times = length(max.both) - length(min.both))
        )
      } else {
        varmax <- c(varmax, rep("-", times = length(varmin) - length(varmax)))
        max.both <- c(
          max.both,
          rep("-", times = length(min.both) - length(max.both))
        )
      }
    }
    ...
    restxt <- data.frame(
      Extrapolation = c(
        ...
        rep(
          "",
          max(length(varmin.combinatorial), length(varmax.combinatorial)) - 1
        ),
        "Both",
  ...
  if (create.plots) {
    ...
    Ax <- purrr::map_dbl(.x = combs, .f = ~ length(.x)) %>%
      table() %>%
      data.frame(.)
    ...
    if (extrapolation.type == "both") {
      res <- data.frame(
        nvars = purrr::map_dbl(combs, ~ length(.)),
        extrap = c(vars.univariate, vars.combinatorial),
        type = rep(c("Univariate", "Combinatorial"), each = length(vars) / 2)
      )
    } else {
      res <- data.frame(
        nvars = purrr::map_dbl(combs, ~ length(.)),
    ...
    if (display.percent) {
      res$extrap <- 100 * res$extrap / ntot
    }
    ...
    if (extrapolation.type == "both") {
      var.ind.univariate <- purrr::map_df(
        .x = var.ind,
        .f = ~ vars.univariate[.]
      ) %>%
        tidyr::gather(data = .)

      var.ind.combinatorial <- purrr::map_df(
        .x = var.ind,
        .f = ~ vars.combinatorial[.]
      ) %>%
        tidyr::gather(data = .)

      var.ind.univariate <- var.ind.univariate %>%
        dplyr::mutate(type = "Univariate")
      var.ind.combinatorial <- var.ind.combinatorial %>%
        dplyr::mutate(type = "Combinatorial")

      varplot <- data.frame(
        extrap = rbind(var.ind.univariate, var.ind.combinatorial)
      )

      if (display.percent) {
        varplot$extrap.value <- 100 * varplot$extrap.value / ntot
      }
      ...
      varplot <- data.frame(
        var = rep(covariate.names, each = length(var.ind[[1]])),
        extrap = unlist(var.ind)
      )

      if (display.percent) {
        varplot$extrap <- 100 * varplot$extrap / ntot
      }
    ...
    labelres <- res %>%
      ...
      dplyr::summarize(
        ypos = max(res$extrap) + 0.15 * max(res$extrap),
        label = unique(label)
      )
    ...
    labelvar <- varplot %>%
      ...
      dplyr::summarize(
        ypos = max(varplot$extrap) + 0.15 * max(varplot$extrap),
        label = unique(label)
      )
    ...
    p1 <- ggplot2::ggplot(
      data = res,
      ggplot2::aes(x = factor(nvars), y = extrap)
    ) +
      ...
      {
        if (extrapolation.type == "combinatorial") {
          geom_boxplot(fill = '#2E86A6')
        }
      } +

      geom_text(
        data = labelres,
        ggplot2::aes(x = factor(nvars), y = ypos, label = label)
      ) +
      ...
      {
        if (extrapolation.type == "both") {
          scale_fill_manual(values = c('#2E86A6', '#F0B039'))
        }
      }

    p2 <- ggplot2::ggplot(
      data = varplot,
      ggplot2::aes(x = factor(var), y = extrap)
    ) +
      ...
      geom_text(
        data = labelvar,
        ggplot2::aes(x = factor(var), y = ypos, label = label)
      ) +
      xlab("Variable") +
      {
        if (display.percent) ylab("Extrapolation (%)")
