...
collapse.vars <- function(l) {
  ...
  if (length(l) > 1) {
    ...
    l <- apply(X = l, MARGIN = 1, function(x) {
      paste(x, collapse = " ") %>%
        trimws(., which = "both") %>%
...
safe_raster <- purrr::safely(
  function(x, crs) suppressWarnings(terra::rast(x, type = 'xyz', crs = crs)),
  otherwise = NULL
)

safe_pts <- purrr::safely(function(x) {
  suppressWarnings(SpatialPointsDataFrame(
    coords = x[, 1:2],
    data = data.frame(x[, 3])
  ))
})
...
rescale_cov <- function(ynew, y) {
  return((ynew - mean(y, na.rm = TRUE)) / (stats::sd(y, na.rm = TRUE)))
}
...
make_X <- function(calibration_data, test_data, var_name) {
  ...
  X <- sapply(var_name, function(k) {
    rescale_cov(ynew = test_data[[k]], y = calibration_data[[k]])
  })
  X <- as.data.frame(X)
  names(X) <- var_name
  return(X)
...
proj_rasters <- function(ll, coordinate.system) {
  ...
  analogue.xy <- raster::as.data.frame(llr$analogue, xy = TRUE) %>%
    stats::na.omit(.)
  analogue.xy <- sp::SpatialPointsDataFrame(
    coords = analogue.xy[, c("x", "y")],
    data = analogue.xy,
  ...
  univariate.xy <- raster::as.data.frame(llr$univariate, xy = TRUE) %>%
    stats::na.omit(.)
  univariate.xy <- sp::SpatialPointsDataFrame(
    coords = univariate.xy[, c("x", "y")],
    data = univariate.xy,
    proj4string = coordinate.system
  )
  ...
  suppressWarnings(
    llr <- purrr::map(
      .x = llr, # Same extent as the full raster, allows correct alignment
      .f = ~ raster::projectRaster(from = .x, to = ll$all, method = 'ngb')
    ) %>%
      purrr::map(
        .x = ., # CRS used by leaflet
        .f = ~ raster::projectRaster(
          from = .,
          crs = crs.webmerc,
          method = 'ngb'
        )
  ...
  llr.univariate.ind <- raster::cellFromXY(
    object = llr$univariate,
    xy = sp::coordinates(univariate.xy)
  )

  llr$univariate[llr.univariate.ind[which(is.na(llr$univariate[
    llr.univariate.ind
  ]))]] <- univariate.values[which(is.na(llr$univariate[llr.univariate.ind]))]
  ...
  llr.analogue.ind <- raster::cellFromXY(
    object = llr$analogue,
    xy = duplicate.cells
  )
...
check_crs <- function(coordinate.system) {
  if (!class(coordinate.system) == "crs") {
    coord.err <- tryCatch(
      expr = sf::st_crs(coordinate.system),
      error = function(e) return(NA)
    )

    if (is.na(coord.err)) {
      stop('Unrecognised coordinate system')
    } else {
      suppressWarnings(coordinate.system <- sf::st_crs(coordinate.system))
    }
  }
