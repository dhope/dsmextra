...
map_extrapolation <- function(
  ...
  if (is.null(map.type)) {
    stop("Argument 'maptype' must be specified.")
  }
  if (!map.type %in% c("extrapolation", "nearby", "mic")) {
    stop("Unknown map type requested.")
  }
  if (is.null(extrapolation.object) & map.type == "extrapolation") {
    stop(
      "Argument 'extrapolation.values' cannot be NULL when maptype is set to 'extrapolation'."
    )
  }
  if (is.null(extrapolation.object) & map.type == "mic") {
    stop(
      "Argument 'extrapolation.values' cannot be NULL when maptype is set to 'mic'."
    )
  }
  if (!map.type %in% extrapolation.object$type) {
    stop("Map type undefined for the input extrapolation.object")
  }
  ...
  if (map.type == "nearby") {
    gower.values <- extrapolation.object$raster
  }
  if (map.type %in% c("extrapolation", "mic")) {
    extrapolation.values <- extrapolation.object
  }

  if (!is.null(sightings)) {
    if (
      !class(sightings) %in%
        c("data.frame", "matrix", "SpatialPoints", "SpatialPointsDataFrame")
    ) {
      stop(
        "Sightings must be of class matrix, data.frame, SpatialPoints or SpatialPointsDataFrame"
      )
    }
    ...
    if (is.data.frame(sightings) | is.matrix(sightings)) {
      ...
      if (!is.null(sightings)) {
        latlon.sightings <- sum(all(
          range(sightings$x) >= -180 & range(sightings$x) <= 180
        ))
      }
    } else {
      coords.sightings <- sf::st_coordinates(sightings) #sp::coordinates(sightings)
      latlon.sightings <- sum(all(
        as.vector(apply(coords.sightings, 2, range)) >= -180 &
          as.vector(apply(coords.sightings, 2, range)) <= 180
      ))
    }
  }
  ...
  if (!is.null(tracks)) {
    if (
      ...
      stop(
        "Tracks must be of class matrix, data.frame, SpatialLines or SpatialLinesDataFrame"
      )
    }
    ...
    if (is.data.frame(tracks) | is.matrix(tracks)) {
      ...
      if (!is.null(tracks)) {
        latlon.tracks <- sum(all(
          range(tracks$x) >= -180 & range(tracks$x) <= 180
        ))
      }
    } else {
      ...
      latlon.tracks <- sum(all(
        as.vector(apply(coords.tracks, 2, range)) >= -180 &
          as.vector(apply(coords.tracks, 2, range)) <= 180
      ))
    }
  }
  ...
  if (map.type %in% c("mic", "extrapolation")) {
    types <- purrr::map_dbl(.x = extrapolation.values$data[2:4], .f = ~ nrow(.))
  ...
  survey_extent <- methods::as(
    raster::extent(range(prediction.grid$x), range(prediction.grid$y)),
    "SpatialPolygons"
  )
  ...
  if (!is.null(tracks)) {
    ...
    if (is.data.frame(tracks) | is.matrix(tracks)) {
      ...
      tracks <- sp::spTransform(tracks, CRSobj = crs.ll)
    }
  }
  ...
  if (!is.null(sightings)) {
    if (latlon.sightings == 1) {
      crssightings <- crs.ll
    } else {
      crssightings <- coordinate.system
    }
    ...
    if (is.data.frame(sightings) | is.matrix(sightings)) {
      ...
      sightings <- sp::spTransform(x = sightings, CRSobj = crs.ll)
    }
  }
  ...
  exleaf <- leaflet::leaflet(sightings) %>%
    leaflet::setView(
      lng = mean(c(survey_extent[1], survey_extent[2])),
  ...
  if (map.type == "extrapolation") {
    ...
    projr <- proj_rasters(
      ll = extrapolation.values$rasters$ExDet,
    ...
    if ("univariate" %in% names(projr)) {
      pal.univariate <- leaflet::colorNumeric(
        c("#7f2704", "#fd8d3c", "#fee6ce"),
        raster::getValues(projr$univariate),
    ...
    if ("combinatorial" %in% names(projr)) {
      if (base.layer == "ocean") {
        pal.combinatorial <- leaflet::colorNumeric(
          c("#eadef7", "#a56bd6", "#47026d"),
          raster::getValues(projr$combinatorial),
          na.color = "transparent"
        )
      } else {
        pal.combinatorial <- leaflet::colorNumeric(
    ...
    if ("analogue" %in% names(projr)) {
      pal.analogue <- leaflet::colorNumeric(
        c("#e5f5e0", "#74c476", "#00441b"),
    ...
    pal <- leaflet::colorFactor(
      palette = c(
        "#B2FF8C",
        dichromat::colorschemes$Categorical.12[c(1:4, 7:12)]
      ),
    ...
    allvars <- tibble::tibble(
      vars = c("None", covariate.names),
      ID = seq(0, length(covariate.names))
    )
    ...
    if ("size" %in% names(sightings)) {
      ...
      exleaf <- exleaf %>%
        leaflet::addCircleMarkers(
          lng = sp::coordinates(sightings)[, 1],
          lat = sp::coordinates(sightings)[, 2],
          radius = ~ (log(sightings$size) + 1) * 3,
          label = sizelist,
          labelOptions = lapply(1:nrow(sightings), function(x) {
  ...
  toggles <- paste0(
    switch(map.type, "extrapolation" = "1", "mic" = "2", "nearby" = "3"),
    paste0(
      as.character(as.numeric(c(!is.null(tracks), !is.null(sightings)))),
      collapse = ""
    )
  )
